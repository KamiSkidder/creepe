...
public class EntityUtill implements Util
{
    ...
    public static void attackEntity(final Entity entity, final boolean packet, final boolean swingArm) {
        if (packet) {
            EntityUtill.mc.player.connection.sendPacket((Packet)new CPacketUseEntity(entity));
        }
        else {
            EntityUtill.mc.playerController.attackEntity((EntityPlayer)EntityUtill.mc.player, entity);
        }
        if (swingArm) {
    ...
    public static Vec3d getInterpolatedRenderPos(final Entity entity, final float partialTicks) {
        return getInterpolatedPos(entity, partialTicks).subtract(EntityUtill.mc.getRenderManager().renderPosX, EntityUtill.mc.getRenderManager().renderPosY, EntityUtill.mc.getRenderManager().renderPosZ);
    }
    ...
    public static double getBaseMoveSpeed() {
        ...
        if (EntityUtill.mc.player != null && EntityUtill.mc.player.isPotionActive(Potion.getPotionById(1))) {
            final int amplifier = EntityUtill.mc.player.getActivePotionEffect(Potion.getPotionById(1)).getAmplifier();
    ...
    public static boolean stopSneaking(final boolean isSneaking) {
        if (isSneaking && EntityUtill.mc.player != null) {
            EntityUtill.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)EntityUtill.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
        }
        return false;
    }
    ...
    public static boolean isMobAggressive(final Entity entity) {
        if (entity instanceof EntityPigZombie) {
            ...
            if (entity instanceof EntityWolf) {
                return ((EntityWolf)entity).isAngry() && !EntityUtill.mc.player.equals((Object)((EntityWolf)entity).getOwner());
            }
    ...
    public static List<Vec3d> getUnsafeBlocksFromVec3d(final Vec3d pos, final int height, final boolean floor) {
        ...
        for (final Vec3d vector : getOffsets(height, floor)) {
            ...
            final Block block = EntityUtill.mc.world.getBlockState(targetPos).getBlock();
    ...
    public static boolean isObbyHole(final BlockPos blockPos) {
        ...
        for (final BlockPos pos : array) {
            final IBlockState touchingState = EntityUtill.mc.world.getBlockState(pos);
    ...
    public static boolean isInWater(final Entity entity) {
        ...
        for (int x = MathHelper.floor(entity.posX); x < MathHelper.ceil(entity.posX); ++x) {
            for (int z = MathHelper.floor(entity.posZ); z < MathHelper.ceil(entity.posZ); ++z) {
                ...
                if (EntityUtill.mc.world.getBlockState(pos).getBlock() instanceof BlockLiquid) {
                    return true;
                }
            }
        }
    ...
    public static boolean isDrivenByPlayer(final Entity entityIn) {
        return EntityUtill.mc.player != null && entityIn != null && entityIn.equals((Object)EntityUtill.mc.player.getRidingEntity());
    }
    ...
    public static List<Vec3d> targets(final Vec3d vec3d, final boolean antiScaffold, final boolean antiStep, final boolean legs, final boolean platform, final boolean antiDrop, final boolean raytrace) {
        final ArrayList<Vec3d> placeTargets = new ArrayList<Vec3d>();
        if (antiDrop) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, EntityUtill.antiDropOffsetList));
        }
        if (platform) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, EntityUtill.platformOffsetList));
        }
        if (legs) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, EntityUtill.legOffsetList));
        }
        Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, EntityUtill.OffsetList));
        if (antiStep) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, EntityUtill.antiStepOffsetList));
        }
        else {
            ...
            if (vec3ds.size() == 4) {
                for (final Vec3d vector : vec3ds) {
                    ...
                    if (antiScaffold) {
                        Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, EntityUtill.antiScaffoldOffsetList));
                    }
                    return placeTargets;
                }
            }
    ...
    public static boolean canEntityFeetBeSeen(final Entity entityIn) {
        return EntityUtill.mc.world.rayTraceBlocks(new Vec3d(EntityUtill.mc.player.posX, EntityUtill.mc.player.posX + EntityUtill.mc.player.getEyeHeight(), EntityUtill.mc.player.posZ), new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ), false, true, false) == null;
    }
    
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)EntityUtill.mc.player) || (entity instanceof EntityPlayer && OyVey.friendManager.isFriend(entity.getName())) || EntityUtill.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (EntityUtill.mc.player.isPotionActive((Potion)Objects.requireNonNull(Potion.getPotionById(1)))) {
            maxModifier *= 1.0 + 0.2 * (Objects.requireNonNull(EntityUtill.mc.player.getActivePotionEffect((Potion)Objects.requireNonNull(Potion.getPotionById(1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static void moveEntityStrafe(final double speed, final Entity entity) {
        if (entity != null) {
            final MovementInput movementInput = EntityUtill.mc.player.movementInput;
    ...
    public static boolean isMoving() {
        return EntityUtill.mc.player.moveForward != 0.0 || EntityUtill.mc.player.moveStrafing != 0.0;
    }
    ...
    public static EntityPlayer getClosestEnemy(final double distance) {
        ...
        for (final EntityPlayer player : EntityUtill.mc.world.playerEntities) {
            ...
            if (closest == null) {
                ...
                if (EntityUtill.mc.player.getDistanceSq((Entity)player) >= EntityUtill.mc.player.getDistanceSq((Entity)closest)) {
                    continue;
                }
                closest = player;
            }
    ...
    public static boolean checkCollide() {
        return !EntityUtill.mc.player.isSneaking() && (EntityUtill.mc.player.getRidingEntity() == null || EntityUtill.mc.player.getRidingEntity().fallDistance < 3.0f) && EntityUtill.mc.player.fallDistance < 3.0f;
    }
    ...
    public static boolean isCrystalAtFeet(final EntityEnderCrystal crystal, final double range) {
        for (final EntityPlayer player : EntityUtill.mc.world.playerEntities) {
            if (EntityUtill.mc.player.getDistanceSq((Entity)player) > range * range) {
                continue;
            }
    ...
    public static double[] forward(final double speed) {
        ...
        float yaw = EntityUtill.mc.player.prevRotationYaw + (EntityUtill.mc.player.rotationYaw - EntityUtill.mc.player.prevRotationYaw) * EntityUtill.mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            final int hpRaw = (int)EntityUtil.getHealth((Entity)player);
