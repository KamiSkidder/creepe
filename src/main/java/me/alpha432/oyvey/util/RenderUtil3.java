...
import me.alpha432.oyvey.features.modules.player.Speedmine3;
...
public class RenderUtil3 implements Util
{
    ...
    public static void boxESP(final BlockPos blockPos, final Color color, final float f, final boolean bl, final boolean bl2, final int n, final boolean bl3) {
        ...
        RenderUtil3.camera.setPosition(Objects.requireNonNull(RenderUtil.mc.getRenderViewEntity()).posX, RenderUtil.mc.getRenderViewEntity().posY, RenderUtil.mc.getRenderViewEntity().posZ);
        if (RenderUtil3.camera.isBoundingBoxInFrustum(new AxisAlignedBB(axisAlignedBB.minX + RenderUtil.mc.getRenderManager().viewerPosX, axisAlignedBB.minY + RenderUtil.mc.getRenderManager().viewerPosY, axisAlignedBB.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, axisAlignedBB.maxX + RenderUtil.mc.getRenderManager().viewerPosX, axisAlignedBB.maxY + RenderUtil.mc.getRenderManager().viewerPosY, axisAlignedBB.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            ...
            float f2 = Speedmine3.getInstance().timer.getPassedTimeMs() / 1000.0f / (Speedmine3.getInstance().breakTime * OyVey.serverManager.getTpsFactor());
            f2 = Math.min(f2, 1.0f);
            double d9;
            double d10;
            double d11;
            ...
            if (bl2) {
                RenderUtil.drawFilledBox(axisAlignedBB2, new Color(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, n / 255.0f).getRGB());
            }
            if (bl) {}
            GL11.glDisable(2848);
    ...
    public static void updateModelViewProjectionMatrix() {
        ...
        GLUProjection.getInstance().updateMatrices(RenderUtil3.viewport, RenderUtil3.modelView, RenderUtil3.projection, res.getScaledWidth() / (float)Minecraft.getMinecraft().displayWidth, res.getScaledHeight() / (float)Minecraft.getMinecraft().displayHeight);
    }
    ...
    public static void drawWaypointImage(final BlockPos pos, final GLUProjection.Projection projection, final Color color, final String name, final boolean rectangle, final Color rectangleColor) {
        ...
        final float textWidth = (float)OyVey.textManager.getStringWidth(text);
        OyVey.textManager.drawString(text, -(textWidth / 2.0f), -(OyVey.textManager.getFontHeight() / 2.0f) + OyVey.textManager.getFontHeight() / 2.0f, color.getRGB(), false);
        GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawGradientPlane(final BlockPos pos, final EnumFacing face, final Color startColor, final Color endColor, final boolean half, final boolean top) {
        ...
        final Vec3d interp = EntityUtil.interpolateEntity((Entity)RenderUtil.mc.player, RenderUtil3.mc.getRenderPartialTicks());
    ...
    public static void drawSexyBoxPhobosIsRetardedFuckYouESP(final AxisAlignedBB a, final Color boxColor, final Color outlineColor, final float lineWidth, final boolean outline, final boolean box, final boolean colorSync, float alpha, final float scale, final float slab) {
        ...
        final AxisAlignedBB bb = RenderUtil.interpolateAxis(new AxisAlignedBB(a.minX + f, a.minY + f + (1.0f - slab), a.minZ + f, a.maxX - f, a.maxY - f, a.maxZ - f));
    ...
    public static void drawBox(final BlockPos pos, final Color color) {
        ...
        RenderUtil3.camera.setPosition(Objects.requireNonNull(RenderUtil.mc.getRenderViewEntity()).posX, RenderUtil.mc.getRenderViewEntity().posY, RenderUtil.mc.getRenderViewEntity().posZ);
        if (RenderUtil3.camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtil.mc.getRenderManager().viewerPosX, bb.minY + RenderUtil.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtil.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtil.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            GlStateManager.pushMatrix();
            GlStateManager.enableBlend();
            GlStateManager.disableDepth();
            GlStateManager.tryBlendFuncSeparate(770, 771, 0, 1);
    ...
    public static void drawBlockOutline(final BlockPos pos, final Color color, final float linewidth, final boolean air) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && RenderUtil.mc.world.getWorldBorder().contains(pos)) {
            final Vec3d interp = EntityUtil.interpolateEntity((Entity)RenderUtil.mc.player, RenderUtil3.mc.getRenderPartialTicks());
            RenderUtil.drawBlockOutline(iblockstate.getSelectedBoundingBox((World)RenderUtil.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
        }
    }
    ...
    public static void drawBlockOutline(final BlockPos pos, final Color color, final float linewidth, final boolean air, final double height, final boolean gradient, final boolean invert, final int alpha) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && RenderUtil.mc.world.getWorldBorder().contains(pos)) {
            ...
            RenderUtil.drawBlockOutline(blockAxis.grow(0.0020000000949949026), color, linewidth);
        }
    }
    ...
    public static void drawOutlinedBlockESP(final BlockPos pos, final Color color, final float linewidth) {
        final IBlockState iblockstate = RenderUtil.mc.world.getBlockState(pos);
        final Vec3d interp = EntityUtil.interpolateEntity((Entity)RenderUtil.mc.player, RenderUtil3.mc.getRenderPartialTicks());
        RenderUtil.drawBoundingBox(iblockstate.getSelectedBoundingBox((World)RenderUtil.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z), linewidth, ColorUtil.toRGBA(color));
    }
    ...
    public static void drawProjectedText(final GLUProjection.Projection projection, final float addX, final float addY, final String text, final Color color, final boolean shadow) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            OyVey.textManager.drawString(text, -(OyVey.textManager.getStringWidth(text) / 2.0f) + addX, addY, color.getRGB(), shadow);
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
        }
    }
    ...
    public static void drawChungusESP(final GLUProjection.Projection projection, final float width, final float height, final ResourceLocation location) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            RenderUtil3.mc.getTextureManager().bindTexture(location);
            GlStateManager.enableTexture2D();
            GlStateManager.disableBlend();
            RenderUtil3.mc.getTextureManager().bindTexture(location);
            drawCompleteImage(0.0f, 0.0f, width, height);
            RenderUtil3.mc.getTextureManager().deleteTexture(location);
            GlStateManager.enableBlend();
            GlStateManager.disableTexture2D();
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
    ...
    public static void drawClock(final float x, final float y, final float radius, final int slices, final int loops, final float lineWidth, final boolean fill, final Color color) {
        ...
        RenderUtil.drawLine(x, y, x + (float)Math.sin(hourAngle * 3.141592653589793 / 180.0) * (radius / 2.0f), y + (float)Math.cos(hourAngle * 3.141592653589793 / 180.0) * (radius / 2.0f), 1.0f, Color.WHITE.getRGB());
        RenderUtil.drawLine(x, y, x + (float)Math.sin(minuteAngle * 3.141592653589793 / 180.0) * (radius - radius / 10.0f), y + (float)Math.cos(minuteAngle * 3.141592653589793 / 180.0) * (radius - radius / 10.0f), 1.0f, Color.WHITE.getRGB());
        RenderUtil.drawLine(x, y, x + (float)Math.sin(secondAngle * 3.141592653589793 / 180.0) * (radius - radius / 10.0f), y + (float)Math.cos(secondAngle * 3.141592653589793 / 180.0) * (radius - radius / 10.0f), 1.0f, Color.RED.getRGB());
    }
    ...
    public static void drawCircleOutline(final float x, final float y, final float radius, final int start, final int end, final int segments) {
        RenderUtil.drawArcOutline(x, y, radius, (float)start, (float)end, segments);
    }
    ...
    public static void drawCircle(final float x, final float y, final float radius, final int start, final int end, final int segments) {
        RenderUtil.drawArc(x, y, radius, (float)start, (float)end, segments);
    }
    ...
    public static void drawOutlinedRoundedRectangle(final int x, final int y, final int width, final int height, final float radius, final float dR, final float dG, final float dB, final float dA, final float outlineWidth) {
        RenderUtil.drawRoundedRectangle((float)x, (float)y, (float)width, (float)height, radius);
        GL11.glColor4f(dR, dG, dB, dA);
        RenderUtil.drawRoundedRectangle(x + outlineWidth, y + outlineWidth, width - outlineWidth * 2.0f, height - outlineWidth * 2.0f, radius);
    }
    ...
    public static boolean isInViewFrustrum(final AxisAlignedBB bb) {
        ...
        RenderUtil3.frustrum.setPosition(Objects.requireNonNull(current).posX, current.posY, current.posZ);
