...
public class AutoCrystal extends Module
{
    ...
    private final Timer3 switchTimer;
    private final Timer3 manualTimer;
    private final Timer3 breakTimer;
    private final Timer3 placeTimer;
    private final Timer3 syncTimer;
    private final Timer3 predictTimer;
    private final Timer3 renderTimer;
    private final AtomicBoolean shouldInterrupt;
    private final Timer3 syncroTimer;
    private final Map<EntityPlayer, Timer3> totemPops;
    ...
    public AutoCrystal() {
        super("CrystalAura", "Best CA on the market", Category.COMBAT, true, false, false);
        this.switchTimer = new Timer3();
        this.manualTimer = new Timer3();
        this.breakTimer = new Timer3();
        this.placeTimer = new Timer3();
        this.syncTimer = new Timer3();
        this.predictTimer = new Timer3();
        this.renderTimer = new Timer3();
        this.shouldInterrupt = new AtomicBoolean(false);
        this.syncroTimer = new Timer3();
        this.totemPops = new ConcurrentHashMap<EntityPlayer, Timer3>();
        ...
        this.placeDelay = (Setting<Integer>)this.register(new Setting("PlaceDelay", 25, (-100), 500, v -> this.setting.getValue() == Settings.PLACE && this.place.getValue()));
        ...
        this.breakDelay = (Setting<Integer>)this.register(new Setting("BreakDelay", 50, (-100), 500, v -> this.setting.getValue() == Settings.BREAK && this.explode.getValue()));
    ...
    public void onPacketSend(final PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)AutoCrystal.mc.world) instanceof EntityEnderCrystal) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet3 = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet3.getEntityFromWorld((World)AutoCrystal.mc.world) instanceof EntityEnderCrystal) {
            ...
            if (this.antiBlock.getValue() && EntityUtill.isCrystalAtFeet(crystal, this.range.getValue()) && pos != null) {
                this.rotateToPos(pos);
                BlockUtil3.placeCrystalOnBlock(this.placePos, this.offHand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, this.placeSwing.getValue(), this.exactHand.getValue(), this.shouldSilent);
            }
        }
    }
    ...
    public void onPacketReceive(final PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue() && this.switchTimer.passedMs(this.switchCooldown.getValue()) && this.explode.getValue() && this.instant.getValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || (this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue())) || (this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue())))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (final EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && OyVey.friendManager.isFriend(friend)) {
                ...
                if (AutoCrystal.placedPos.contains(pos.down())) {
                    ...
                    if (selfDamage + 1.0 < EntityUtil.getHealth((Entity)AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            if (player.getDistanceSq(pos) <= MathUtil.square(this.range.getValue()) && EntityUtil.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue()) && (!this.antiNaked.getValue() || !DamageUtill.isNaked(player))) {
                                final float damage;
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity((World)AutoCrystal.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer)packet5.getEntity((World)AutoCrystal.mc.world), new Timer3().reset());
            }
        }
        else {
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        if ((this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC) && this.renderPos != null && (this.box.getValue() || this.outline.getValue())) {
            ...
            if (this.renderMode.getValue() == RenderMode.STATIC) {
                RenderUtil3.drawSexyBoxPhobosIsRetardedFuckYouESP(new AxisAlignedBB(this.renderPos), boxC, outlineC, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.colorSync.getValue(), 1.0f, 1.0f, this.slabHeight.getValue());
            }
            if (this.renderMode.getValue() == RenderMode.GLIDE) {
                ...
                RenderUtil3.drawSexyBoxPhobosIsRetardedFuckYouESP(this.renderBB = this.renderBB.offset(xDiff * multiplier, yDiff * multiplier, zDiff * multiplier), boxC, outlineC, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.colorSync.getValue(), 1.0f, 1.0f, this.slabHeight.getValue());
                if (this.text.getValue()) {
                    RenderUtil3.drawText(this.renderBB.offset(0.0, 1.0f - this.slabHeight.getValue() / 2.0f - 0.4, 0.0), ((Math.floor(this.renderDamage) == this.renderDamage) ? Integer.valueOf((int)this.renderDamage) : String.format("%.1f", this.renderDamage)) + "");
                }
        ...
        if (this.renderMode.getValue() == RenderMode.FADE) {
            float factor;
            //??
            final Color boxColor = new Color(this.bRed.getValue(), this.bGreen.getValue(), this.bBlue.getValue(), this.bAlpha.getValue());
            final Color outlineColor = new Color(this.oRed.getValue(), this.oGreen.getValue(), this.oBlue.getValue(), this.oAlpha.getValue());
            for (final RenderPos pos : this.positions) {
                factor = (this.duration.getValue() - pos.getRenderTime()) / this.duration.getValue();
                RenderUtil3.drawSexyBoxPhobosIsRetardedFuckYouESP(new AxisAlignedBB(pos.getPos()), boxColor, outlineColor, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.colorSync.getValue(), ((boolean)this.fadeFactor.getValue()) ? factor : 1.0f, ((boolean)this.scaleFactor.getValue()) ? factor : 1.0f, ((boolean)this.slabFactor.getValue()) ? factor : 1.0f);
        ...
        if ((this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC) && this.renderPos != null && this.text.getValue() && this.renderMode.getValue() != RenderMode.GLIDE) {
            RenderUtil3.drawText(new AxisAlignedBB(this.renderPos).offset(0.0, (this.renderMode.getValue() != RenderMode.FADE) ? (1.0f - this.slabHeight.getValue() / 2.0f - 0.4) : 0.1, 0.0), ((Math.floor(this.renderDamage) == this.renderDamage) ? Integer.valueOf((int)this.renderDamage) : String.format("%.1f", this.renderDamage)) + "");
        }
    }
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof OyVeyGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue() && this.offhandSwitch.getValue() && this.offHand) {
                final Offhand module = OyVey.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
    ...
    private boolean check() {
        ...
        if ((!this.offHand && !this.mainHand && this.switchMode.getValue() == Switch.BREAKSLOT && !this.switching) || !DamageUtill.canBreakWeakness((EntityPlayer)AutoCrystal.mc.player) || !this.switchTimer.passedMs(this.switchCooldown.getValue())) {
            this.renderPos = null;
    ...
    private void mapCrystals() {
        ...
        for (final Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            if (!entity.isDead && entity instanceof EntityEnderCrystal) {
                ...
                if (this.syncedFeetPlace.getValue() && entity.getPosition().down().equals((Object)this.syncedCrystalPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    float selfDamage = -1.0f;
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtill.calculateDamage(entity, (Entity)AutoCrystal.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtil.getHealth((Entity)AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        ...
                        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            ...
                            if (EntityUtill.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue())) {
                                if (this.antiNaked.getValue() && DamageUtill.isNaked(player)) {
                                    continue;
                                }
                                final float damage;
                                if ((damage = DamageUtill.calculateDamage(entity, (Entity)player)) <= selfDamage && (damage <= this.minDamage.getValue() || DamageUtill.canTakeDamage(this.suicide.getValue())) && damage <= EntityUtill.getHealth((Entity)player)) {
                                    continue;
                                }
                                if (damage > maxDamage) {
                                    maxDamage = damage;
                                ...
                                if ((this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL) || !OyVey.friendManager.isFriend(player.getName())) {
                                    continue;
                                }
                                if (DamageUtill.calculateDamage(entity, (Entity)player) <= EntityUtill.getHealth((Entity)player) + 0.5) {
                                    continue;
                                }
        ...
        if (this.webAttack.getValue() && this.webPos != null) {
            if (AutoCrystal.mc.player.getDistanceSq(this.webPos.up()) > MathUtil.square(this.breakRange.getValue())) {
                ...
                for (final Entity entity : AutoCrystal.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(this.webPos.up()))) {
                    if (!(entity instanceof EntityEnderCrystal)) {
                        continue;
                    }
    ...
    private void calculateDamage(final EntityPlayer targettedPlayer) {
        ...
        for (final BlockPos pos : BlockUtill.possiblePlacePositions(this.placeRange.getValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtil.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && AutoCrystal.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue()), 0.5f)) {
                continue;
            }
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (final EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtil.square(this.range.getValue() + this.placeRange.getValue()) && OyVey.friendManager.isFriend(friend)) {
                            if (DamageUtill.calculateDamage(pos, (Entity)friend) <= EntityUtil.getHealth((Entity)friend) + 0.5) {
                                continue;
                            }
                            friendPop = true;
                            break;
                ...
                for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    if (EntityUtil.isValid((Entity)player, this.placeRange.getValue() + this.range.getValue())) {
                        ...
                        if ((this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE) || player == null || player.getDistanceSq(pos) > MathUtil.square(this.range.getValue() + this.placeRange.getValue()) || !OyVey.friendManager.isFriend(player)) {
                            continue;
                        }
    ...
    private EntityPlayer getTarget(final boolean unsafe) {
        ...
        EntityPlayer currentTarget = null;
        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
            if (!EntityUtil.isntValid((Entity)player, this.placeRange.getValue() + this.range.getValue()) && (!this.antiNaked.getValue() || !DamageUtill.isNaked(player))) {
        ...
        if (this.predictPos.getValue() && currentTarget != null) {
            ...
            final Vec3d extrapolatePosition = MathUtill.extrapolatePlayerPosition(currentTarget, this.predictTicks.getValue());
    ...
    private void attackEntity(final Entity entity) {
        if (entity != null) {
            if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE && this.rotateFirst.getValue() && this.rotate.getValue() != Rotate.OFF) {
                ...
                EntityUtil3.OffhandAttack(entity, this.attackOppositeHand.getValue(), this.attackOppositeHand.getValue());
    ...
    private void doFakeSwing() {
        if (this.fakeSwing.getValue()) {
            EntityUtil3.swingArmNoPacket(EnumHand.MAIN_HAND, (EntityLivingBase)AutoCrystal.mc.player);
        }
    }
    ...
    private void manualBreaker() {
        if (this.rotate.getValue() != Rotate.OFF && this.eventMode.getValue() != 2 && this.rotating) {
            if (this.didRotation) {
                AutoCrystal.mc.player.rotationPitch += (float)0.1;
            ...
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    ...
                    for (final Entity target : AutoCrystal.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(mousePos))) {
                        ...
                        EntityUtil3.OffhandAttack(target, this.attackOppositeHand.getValue(), this.attackOppositeHand.getValue());
                        this.manualTimer.reset();
                    }
                    break;
                }
    ...
    private boolean isDoublePoppable(final EntityPlayer player, final float damage) {
        ...
        if (this.doublePop.getValue() && (health = EntityUtil.getHealth((Entity)player)) <= this.popHealth.getValue() && damage > health + 0.5 && damage <= this.popDamage.getValue()) {
            final Timer3 timer = this.totemPops.get(player);
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (OyVey.eventManager2.ticksOngoing()) {}
