...
public class RenderUtil2 implements Util
{
    ...
    public static void boxESP(final BlockPos blockPos, final Color color, final float f, final boolean bl, final boolean bl2, final int n, final boolean bl3) {
        ...
        RenderUtil2.camera.setPosition(Objects.requireNonNull(RenderUtil.mc.getRenderViewEntity()).posX, RenderUtil.mc.getRenderViewEntity().posY, RenderUtil.mc.getRenderViewEntity().posZ);
        if (RenderUtil2.camera.isBoundingBoxInFrustum(new AxisAlignedBB(axisAlignedBB.minX + RenderUtil.mc.getRenderManager().viewerPosX, axisAlignedBB.minY + RenderUtil.mc.getRenderManager().viewerPosY, axisAlignedBB.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, axisAlignedBB.maxX + RenderUtil.mc.getRenderManager().viewerPosX, axisAlignedBB.maxY + RenderUtil.mc.getRenderManager().viewerPosY, axisAlignedBB.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
    ...
    public static void updateModelViewProjectionMatrix() {
        ...
        GLUProjection.getInstance().updateMatrices(RenderUtil2.viewport, RenderUtil2.modelView, RenderUtil2.projection, res.getScaledWidth() / (float)Minecraft.getMinecraft().displayWidth, res.getScaledHeight() / (float)Minecraft.getMinecraft().displayHeight);
    }
    ...
    public static void drawGradientPlane(final BlockPos pos, final EnumFacing face, final Color startColor, final Color endColor, final boolean half, final boolean top) {
        ...
        final Vec3d interp = EntityUtil.interpolateEntity((Entity)RenderUtil.mc.player, RenderUtil2.mc.getRenderPartialTicks());
    ...
    public static void drawBox(final BlockPos pos, final Color color) {
        ...
        RenderUtil2.camera.setPosition(Objects.requireNonNull(RenderUtil.mc.getRenderViewEntity()).posX, RenderUtil.mc.getRenderViewEntity().posY, RenderUtil.mc.getRenderViewEntity().posZ);
        if (RenderUtil2.camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtil.mc.getRenderManager().viewerPosX, bb.minY + RenderUtil.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtil.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtil.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            GlStateManager.pushMatrix();
            GlStateManager.enableBlend();
            GlStateManager.disableDepth();
            GlStateManager.tryBlendFuncSeparate(770, 771, 0, 1);
    ...
    public static void drawChungusESP(final GLUProjection.Projection projection, final float width, final float height, final ResourceLocation location) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            RenderUtil2.mc.getTextureManager().bindTexture(location);
            GlStateManager.enableTexture2D();
            GlStateManager.disableBlend();
            RenderUtil2.mc.getTextureManager().bindTexture(location);
            drawCompleteImage(0.0f, 0.0f, width, height);
            RenderUtil2.mc.getTextureManager().deleteTexture(location);
            GlStateManager.enableBlend();
            GlStateManager.disableTexture2D();
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
    ...
    public static boolean isInViewFrustrum(final AxisAlignedBB bb) {
        ...
        RenderUtil2.frustrum.setPosition(Objects.requireNonNull(current).posX, current.posY, current.posZ);
