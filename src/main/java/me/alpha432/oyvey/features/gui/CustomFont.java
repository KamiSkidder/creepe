...
public class CustomFont extends CFont
{
    ...
    public float drawString(final String textIn, final double xI, final double yI, int color, final boolean shadow) {
        double x = xI;
        double y = yI;
        if (shadow) {
            x -= 0.5;
        ...
        if (render) {
            ...
            final int size = textIn.length();
            GlStateManager.enableTexture2D();
            GlStateManager.bindTexture(this.tex.getGlTextureId());
            GL11.glBindTexture(3553, this.tex.getGlTextureId());
            for (int i = 0; i < size; ++i) {
                final char character = textIn.charAt(i);
                if (character == '§' && i < size) {
                    int colorIndex = 21;
                    try {
