...
public class ConfigManager implements Util
{
    ...
    public void loadConfig(final String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("crepe").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        ...
        for (final Setting setting : feature.getSettings()) {
            if (setting.isEnumSetting()) {
                ...
                if (setting.isStringSetting()) {
                    final String str = setting.getValue().toString();
                    setting.setValue(str.replace(" ", "_"));
                }
                try {
