...
public class BlockUtill implements Util
{
    ...
    public static List<BlockPos> getBlockSphere(final float breakRange, final Class clazz) {
        ...
        positions.addAll((Collection)getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtill.mc.player), breakRange, (int)breakRange, false, true, 0).stream().filter(pos -> clazz.isInstance(BlockUtill.mc.world.getBlockState(pos).getBlock())).collect(Collectors.toList()));
    ...
    private static void ProcessRightClickBlock(final BlockPos neighbor, final EnumFacing side2, final Vec3d hitVec) {
    }
    ...
    public static List<EnumFacing> getPossibleSides(final BlockPos pos) {
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (BlockUtill.mc.world.getBlockState(neighbour).getBlock().canCollideCheck(BlockUtill.mc.world.getBlockState(neighbour), false)) {
                final IBlockState blockState;
                if (!(blockState = BlockUtill.mc.world.getBlockState(neighbour)).getMaterial().isReplaceable()) {
                    facings.add(side);
                }
            }
        }
    ...
    public static EnumFacing getRayTraceFacing(final BlockPos pos) {
        final RayTraceResult result = BlockUtill.mc.world.rayTraceBlocks(new Vec3d(BlockUtill.mc.player.posX, BlockUtill.mc.player.posY + BlockUtill.mc.player.getEyeHeight(), BlockUtill.mc.player.posZ), new Vec3d(pos.getX() + 0.5, pos.getX() - 0.5, pos.getX() + 0.5));
    ...
    public static int isPositionPlaceable(final BlockPos pos, final boolean rayTrace, final boolean entityCheck) {
        final Block block = BlockUtill.mc.world.getBlockState(pos).getBlock();
        ...
        if (entityCheck) {
            for (final Entity entity : BlockUtill.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos))) {
    ...
    public static void rightClickBlock(final BlockPos pos, final Vec3d vec, final EnumHand hand, final EnumFacing direction, final boolean packet) {
        if (packet) {
            ...
            BlockUtill.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos, direction, hand, f, f2, f3));
        }
        else {
    ...
    public static boolean placeBlock(final BlockPos pos, final EnumHand hand, final boolean rotate, final boolean packet, final boolean isSneaking) {
        ...
        final Block neighbourBlock = BlockUtill.mc.world.getBlockState(neighbour).getBlock();
        if (!BlockUtill.mc.player.isSneaking() && (BlockUtill.blackList.contains(neighbourBlock) || BlockUtill.shulkerList.contains(neighbourBlock))) {
            BlockUtill.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)BlockUtill.mc.player, CPacketEntityAction.Action.START_SNEAKING));
    ...
    public static void placeBlockStopSneaking(final BlockPos pos, final EnumHand hand, final boolean rotate, final boolean packet, final boolean isSneaking) {
        ...
        if (!isSneaking && sneaking) {
            BlockUtill.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)BlockUtill.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
        }
    }
    ...
    public static List<BlockPos> possiblePlacePositions(final float placeRange) {
        final NonNullList positions = NonNullList.create();
        positions.addAll((Collection)getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtill.mc.player), placeRange, (int)placeRange, false, true, 0).stream().filter(BlockUtill::canPlaceCrystal).collect(Collectors.toList()));
        return (List<BlockPos>)positions;
    }
    ...
    public static List<BlockPos> possiblePlacePositions(final float placeRange, final boolean specialEntityCheck, final boolean oneDot15) {
        final NonNullList positions = NonNullList.create();
        positions.addAll((Collection)getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtill.mc.player), placeRange, (int)placeRange, false, true, 0).stream().filter(pos -> canPlaceCrystal(pos, specialEntityCheck, oneDot15)).collect(Collectors.toList()));
        return (List<BlockPos>)positions;
    }
    ...
    public static boolean canPlaceCrystal(final BlockPos blockPos, final boolean specialEntityCheck, final boolean oneDot15) {
        ...
        try {
            ...
            for (final Entity entity : BlockUtill.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost))) {
                if (!entity.isDead) {
            ...
            if (!oneDot15) {
                for (final Entity entity : BlockUtill.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2))) {
                    if (!entity.isDead) {
                        if (specialEntityCheck && entity instanceof EntityEnderCrystal) {
                            continue;
                        }
    ...
    public static void placeCrystalOnBlock(final BlockPos pos, final EnumHand hand, final boolean swing, final boolean exactHand) {
        final RayTraceResult result = BlockUtill.mc.world.rayTraceBlocks(new Vec3d(BlockUtill.mc.player.posX, BlockUtill.mc.player.posY + BlockUtill.mc.player.getEyeHeight(), BlockUtill.mc.player.posZ), new Vec3d(pos.getX() + 0.5, pos.getY() - 0.5, pos.getZ() + 0.5));
        final EnumFacing facing = (result == null || result.sideHit == null) ? EnumFacing.UP : result.sideHit;
        BlockUtill.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos, facing, hand, 0.0f, 0.0f, 0.0f));
        if (swing) {
            BlockUtill.mc.player.connection.sendPacket((Packet)new CPacketAnimation(exactHand ? hand : EnumHand.MAIN_HAND));
        }
    }
    ...
    public static Boolean isPosInFov(final BlockPos pos) {
        ...
        if (dirnumber == 0 && pos.getZ() - BlockUtill.mc.player.getPositionVector().z < 0.0) {
            return false;
        }
        if (dirnumber == 1 && pos.getX() - BlockUtill.mc.player.getPositionVector().x > 0.0) {
            return false;
        }
        if (dirnumber == 2 && pos.getZ() - BlockUtill.mc.player.getPositionVector().z > 0.0) {
            return false;
        }
        return dirnumber != 3 || pos.getX() - BlockUtill.mc.player.getPositionVector().x >= 0.0;
    }
    ...
    public static boolean canBreak(final BlockPos pos) {
        final IBlockState blockState = BlockUtill.mc.world.getBlockState(pos);
        final Block block = blockState.getBlock();
        return block.getBlockHardness(blockState, (World)BlockUtill.mc.world, pos) != -1.0f;
    }
    ...
    public static boolean rayTracePlaceCheck(final BlockPos pos, final boolean shouldCheck, final float height) {
        return !shouldCheck || BlockUtill.mc.world.rayTraceBlocks(new Vec3d(BlockUtill.mc.player.posX, BlockUtill.mc.player.posY + BlockUtill.mc.player.getEyeHeight(), BlockUtill.mc.player.posZ), new Vec3d((double)pos.getX(), (double)(pos.getY() + height), (double)pos.getZ()), false, true, false) == null;
    }
