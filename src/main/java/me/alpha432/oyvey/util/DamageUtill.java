...
public class DamageUtill implements Util
{
    ...
    public static boolean canBreakWeakness(final EntityPlayer player) {
        int strengthAmp = 0;
        final PotionEffect effect = DamageUtill.mc.player.getActivePotionEffect(MobEffects.STRENGTH);
        if (effect != null) {
    ...
    public static float calculateDamage(final double posX, final double posY, final double posZ, final Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = getBlastReduction((EntityLivingBase)entity, getDamageMultiplied(damage), new Explosion((World)DamageUtill.mc.world, (Entity)null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float)finald;
    }
    ...
    public static float getDamageMultiplied(final float damage) {
        final int diff = DamageUtill.mc.world.getDifficulty().getId();
