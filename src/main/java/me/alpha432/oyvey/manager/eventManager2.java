...
public class eventManager2 extends Feature
{
    ...
    public void onUpdate(final LivingEvent.LivingUpdateEvent event) {
        if (!Feature.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals((Object)eventManager2.mc.player)) {
    ...
    public void onPacketReceive(final PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketPlayerListItem && !Feature.fullNullCheck() && this.logoutTimer.passedS(1.0)) {
            ...
            UUID id;
            SPacketPlayerListItem sPacketPlayerListItem = event.getPacket();
            String name;
            EntityPlayer entity;
            String logoutName;
            List<SPacketPlayerListItem.AddPlayerData> packet = packet2.getEntries().stream().filter(Objects::nonNull).filter(data -> !Strings.isNullOrEmpty(data.getProfile().getName()) || data.getProfile().getId() != null).collect(Collectors.toList());
            for (final SPacketPlayerListItem.AddPlayerData data : packet) {
