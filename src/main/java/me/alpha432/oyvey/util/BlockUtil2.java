...
public class BlockUtil2 implements Util
{
    ...
    public static List<BlockPos> getBlockSphere(final float breakRange, final Class clazz) {
        ...
        positions.addAll((Collection)BlockUtil.getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtil2.mc.player), breakRange, (int)breakRange, false, true, 0).stream().filter(pos -> clazz.isInstance(BlockUtil2.mc.world.getBlockState(pos).getBlock())).collect(Collectors.toList()));
    ...
    public static EnumFacing getFacing(final BlockPos pos) {
        for (final EnumFacing facing : EnumFacing.values()) {
            final RayTraceResult rayTraceResult = BlockUtil2.mc.world.rayTraceBlocks(new Vec3d(BlockUtil2.mc.player.posX, BlockUtil2.mc.player.posY + BlockUtil2.mc.player.getEyeHeight(), BlockUtil2.mc.player.posZ), new Vec3d(pos.getX() + 0.5 + facing.getDirectionVec().getX() * 1.0 / 2.0, pos.getY() + 0.5 + facing.getDirectionVec().getY() * 1.0 / 2.0, pos.getZ() + 0.5 + facing.getDirectionVec().getZ() * 1.0 / 2.0), false, true, false);
            if (rayTraceResult == null || (rayTraceResult.typeOfHit == RayTraceResult.Type.BLOCK && rayTraceResult.getBlockPos().equals((Object)pos))) {
                return facing;
            }
        }
        if (pos.getY() > BlockUtil2.mc.player.posY + BlockUtil2.mc.player.getEyeHeight()) {
    ...
    public static List<EnumFacing> getPossibleSides(final BlockPos pos) {
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            final IBlockState blockState = BlockUtil2.mc.world.getBlockState(neighbour);
    ...
    public static EnumFacing getRayTraceFacing(final BlockPos pos) {
        final RayTraceResult result = BlockUtil2.mc.world.rayTraceBlocks(new Vec3d(BlockUtil2.mc.player.posX, BlockUtil2.mc.player.posY + BlockUtil2.mc.player.getEyeHeight(), BlockUtil2.mc.player.posZ), new Vec3d(pos.getX() + 0.5, pos.getX() - 0.5, pos.getX() + 0.5));
    ...
    public static int isPositionPlaceable(final BlockPos pos, final boolean rayTrace, final boolean entityCheck) {
        final Block block = BlockUtil2.mc.world.getBlockState(pos).getBlock();
        ...
        if (entityCheck) {
            for (final Entity entity : BlockUtil2.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos))) {
    ...
    public static void rightClickBlock(final BlockPos pos, final Vec3d vec, final EnumHand hand, final EnumFacing direction, final boolean packet) {
        if (packet) {
            ...
            BlockUtil2.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos, direction, hand, f, f2, f3));
        }
        else {
    ...
    public static void rightClickBed(final BlockPos pos, final float range, final boolean rotate, final EnumHand hand, final AtomicDouble yaw, final AtomicDouble pitch, final AtomicBoolean rotating, final boolean packet) {
        ...
        final RayTraceResult result = BlockUtil2.mc.world.rayTraceBlocks(new Vec3d(BlockUtil2.mc.player.posX, BlockUtil2.mc.player.posY + BlockUtil2.mc.player.getEyeHeight(), BlockUtil2.mc.player.posZ), posVec);
    ...
    public static void rightClickBlockLegit(final BlockPos pos, final float range, final boolean rotate, final EnumHand hand, final AtomicDouble Yaw2, final AtomicDouble Pitch, final AtomicBoolean rotating, final boolean packet) {
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (distanceSqHitVec <= MathUtil.square(range) && distanceSqHitVec < distanceSqPosVec && BlockUtil2.mc.world.rayTraceBlocks(eyesPos, hitVec, false, true, false) == null) {
                if (rotate) {
    ...
    public static boolean placeBlock(final BlockPos pos, final EnumHand hand, final boolean rotate, final boolean packet, final boolean isSneaking) {
        ...
        final Block neighbourBlock = BlockUtil2.mc.world.getBlockState(neighbour).getBlock();
    ...
    public static List<BlockPos> possiblePlacePositions(final float placeRange) {
        final NonNullList positions = NonNullList.create();
        positions.addAll((Collection)BlockUtil.getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtil2.mc.player), placeRange, (int)placeRange, false, true, 0).stream().filter(BlockUtil2::canPlaceCrystal).collect(Collectors.toList()));
        return (List<BlockPos>)positions;
    }
    ...
    public static List<BlockPos> possiblePlacePositions(final float placeRange, final boolean specialEntityCheck, final boolean oneDot15) {
        final NonNullList positions = NonNullList.create();
        positions.addAll((Collection)BlockUtil.getSphere(EntityUtil.getPlayerPos((EntityPlayer)BlockUtil2.mc.player), placeRange, (int)placeRange, false, true, 0).stream().filter(pos -> canPlaceCrystal(pos, specialEntityCheck, oneDot15)).collect(Collectors.toList()));
        return (List<BlockPos>)positions;
    }
    ...
    public static boolean canPlaceCrystal(final BlockPos blockPos, final boolean specialEntityCheck, final boolean oneDot15) {
        ...
        try {
            ...
            if ((!oneDot15 && BlockUtil2.mc.world.getBlockState(boost2).getBlock() != Blocks.AIR) || BlockUtil2.mc.world.getBlockState(boost).getBlock() != Blocks.AIR) {
                return false;
            }
            for (final Entity entity : BlockUtil2.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost))) {
                if (!entity.isDead) {
            ...
            if (!oneDot15) {
                for (final Entity entity : BlockUtil2.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2))) {
                    if (!entity.isDead) {
                        if (specialEntityCheck && entity instanceof EntityEnderCrystal) {
                            continue;
                        }
    ...
    public static Boolean isPosInFov(final BlockPos pos) {
        ...
        if (dirnumber == 0 && pos.getZ() - BlockUtil2.mc.player.getPositionVector().z < 0.0) {
            return false;
        }
        if (dirnumber == 1 && pos.getX() - BlockUtil2.mc.player.getPositionVector().x > 0.0) {
            return false;
        }
        if (dirnumber == 2 && pos.getZ() - BlockUtil2.mc.player.getPositionVector().z > 0.0) {
            return false;
        }
        return dirnumber != 3 || pos.getX() - BlockUtil2.mc.player.getPositionVector().x >= 0.0;
    }
    ...
    public static boolean canBreak(final BlockPos pos) {
        final IBlockState blockState = BlockUtil2.mc.world.getBlockState(pos);
        final Block block = blockState.getBlock();
        return block.getBlockHardness(blockState, (World)BlockUtil2.mc.world, pos) != -1.0f;
    }
    ...
    public static boolean rayTracePlaceCheck(final BlockPos pos, final boolean shouldCheck, final float height) {
        return !shouldCheck || BlockUtil2.mc.world.rayTraceBlocks(new Vec3d(BlockUtil2.mc.player.posX, BlockUtil2.mc.player.posY + BlockUtil2.mc.player.getEyeHeight(), BlockUtil2.mc.player.posZ), new Vec3d((double)pos.getX(), (double)(pos.getY() + height), (double)pos.getZ()), false, true, false) == null;
    }
    ...
    public static boolean isBlockValid(final IBlockState blockState, final BlockPos blockPos) {
        return blockState.getBlock() == Blocks.AIR && BlockUtil2.mc.player.getDistanceSq(blockPos) >= 1.0 && BlockUtil2.mc.world.getBlockState(blockPos.up()).getBlock() == Blocks.AIR && BlockUtil2.mc.world.getBlockState(blockPos.up(2)).getBlock() == Blocks.AIR && (isBedrockHole(blockPos) || isObbyHole(blockPos) || isBothHole(blockPos) || isElseHole(blockPos));
    }
    
    public static boolean isObbyHole(final BlockPos blockPos) {
        for (final BlockPos pos : getTouchingBlocks(blockPos)) {
            final IBlockState touchingState = BlockUtil2.mc.world.getBlockState(pos);
